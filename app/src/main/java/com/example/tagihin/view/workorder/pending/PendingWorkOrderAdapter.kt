...
class PendingWorkOrderAdapter(
    var multiSelectMode: Boolean = false,
    var context: Context,
    val onClickListener: PendingWorkOrderOnClickListener,
    private val retryCallback: () -> Unit,
    val multiSelectSupport: Boolean = false,
    diffCallback: DiffUtil.ItemCallback<PendingWorkOrder>
) : PagedListAdapter<PendingWorkOrder, RecyclerView.ViewHolder>(diffCallback) {
    ...
    inner class BillViewHolder(
        val binding: BillItemLayoutBinding, val onClickListener: PendingWorkOrderOnClickListener
    ) : RecyclerView.ViewHolder(
        binding.root
    ) {
        fun onBind(bill: PendingWorkOrder) {
            with(binding) {
...
interface PendingWorkOrderOnClickListener{
    fun onClick(bill: PendingWorkOrder)
