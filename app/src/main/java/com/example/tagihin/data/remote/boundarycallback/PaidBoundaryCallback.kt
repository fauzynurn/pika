...
class PaidBoundaryCallback(
    private val webservice: ApiService,
    private val handleResponse: (data: MutableList<PaidBill>) -> Unit,
    private val ioExecutor: Executor,
    private val pref: PreferencesHelper
) : PagedList.BoundaryCallback<PaidBill>() {
    ...
    override fun onZeroItemsLoaded() {
        helper.runIfNotRunning(PagingRequestHelper.RequestType.INITIAL) { helper ->
            webservice.getPaidBill(pref.getUsername()!!, 3, 0, 10)
     ...
     */
    @MainThread
    override fun onItemAtEndLoaded(itemAtEnd: PaidBill) {
        helper.runIfNotRunning(PagingRequestHelper.RequestType.AFTER) { helper ->
            ...
            webservice.getPaidBill(pref.getUsername()!!, 3, index, 10)
                .subscribeOn(Schedulers.io())
                .subscribe({
                    insertItemsIntoDb(it.body()?.data, helper)
                }, {
    ...
    override fun onItemAtFrontLoaded(itemAtFront: PaidBill) {
