...
import com.example.tagihin.data.remote.boundarycallback.UnpaidWorkOrderBoundaryCallback
...
class UnpaidWorkOrderRepository (val apiService : ApiService,
     ...
     */
    private fun insertUnpaidResultIntoDb(body: MutableList<UnpaidWorkOrder>?) {
        body.let { bills ->
            db.runInTransaction {
                if (bills != null) {
                    db.posts().insertUnpaidWorkOrder(bills)
                }
            }
        }
    }
    ...
    private fun refresh(): LiveData<NetworkState> {
        ...
        apiService.getUnpaidWorkOrderBill(pref.getUsername()!!,3,0,10)
            ...
            .subscribe({
                ioExecutor.execute {
                    db.runInTransaction {
                        db.posts().deleteAllUnpaidWorkOrder()
                        insertUnpaidResultIntoDb(it?.body()?.data?.toMutableList())
                    }
    ...
    fun getUnpaidWorkOrder(pageSize: Int): Listing<UnpaidWorkOrder> {
        val boundaryCallback =
            UnpaidWorkOrderBoundaryCallback(
        ...
        val livePagedList = db.posts().getUnpaidWorkOrder().toLiveData(
            pageSize = pageSize,
